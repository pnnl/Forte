...
const y = d3.scaleLinear()
  .domain([0, 1.10*d3.max(data2, function(d) { return +d.MAE; })])
  .range([ height, 0 ]);
...
// Add Y axis
const y = d3.scaleLinear()
  .domain([0, 1.10*d3.max(data2, function(d) { return +d.MAPE; })])
  .range([ height, 0 ]);
svg.selectAll(".g_y").data([0]).join("g").attr("class", "g_y")
  //svg.append("g")
  .call(d3.axisLeft(y));
        ...
        d3.csv(path2).then(function(data2){
          d3.csv(path3).then(function(data3){
            d3.csv(path4).then(function(data4){
              ...
              var svg1 = d3.select("#my_dataviz_svg_monthly")
              //.append("svg")
              .attr("width", parent_width + margin.left + margin.right)
              ...
              p.each(function(d, i){
                  ...
                  var unfiltered_data = (this_metric === "mae")?data2:data4;

                  element.selectAll(".title_text").data([0]).join("text").attr("class", "title_text")
                      .attr("x", (width / 2))             
                      .attr("y", 0 + (margin.top / 6.65))
                      .attr("text-anchor", "middle")  
                      .style("font-size", "16px")  
                      .text(this_month+" "+this_metric.toUpperCase());
                  ...
                  element.selectAll(".x_axis_label").data([0]).join("text").attr("class", "x_axis_label")
                    .attr("text-anchor", "end")
                    .attr("x", width/1.8)
                    .attr("y", height+40)
                    .text("Noise Percentage(%)");  
