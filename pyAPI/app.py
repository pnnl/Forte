...
def processor(start_date="2020-05-01 00:00:00", end_date="2020-05-03 00:00:00", solar_penetration=50):
    ...
    # preparing df
    net_load = ((y_pred.flatten()).tolist())
    net_load.extend(Y_test.tolist())
    net_load_type = (["predicted"] * y_pred.size)
    net_load_type.extend(["actual"] * Y_test.size)
    years = (list(range(1,y_pred.size+1)))
    years.extend(list(range(1,Y_test.size+1)))
