...
class Plots extends Component {
    ...
    componentDidMount() {
        ...
        this.getTitle();
        this.plot_output_mae_multi(this.props.selected_job_name_sa, this.props.url)
        this.plot_output_mape(this.props.selected_job_name_sa, this.props.url)
    }
    ...
    plotTitle(selected_job_name_sa, url){
      var svg = d3.select("#my_dataviz_svg_title");
      ...
      d3.text(url+"/outputs/jobs/"+selected_job_name_sa+"/title.txt").then(function(data1){
        the_title = data1; 
        console.log(the_title);
        svg.selectAll(".title_text1").data([0]).join("text").attr("class", "title_text1")
            .attr("x", (width / 2))             
    ...
    plot_output_mae_multi(selected_job_name_sa, url){
